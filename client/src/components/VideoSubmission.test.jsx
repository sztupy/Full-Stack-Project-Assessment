...
describe("Submitting a new video", () => {
	beforeEach(async () => {
		addVideoMock = vi.fn();
	...
	describe("Validation", () => {
		...
		it("Marks the url box as invalid if it's not a youtube url", () => {
			...
			fireEvent.change(input, {
				target: { value: "https://codeyourfuture.io" },
			});
	...
	describe("Action", () => {
		it("Sends valid input when submitting", () => {
			...
			expect(addVideoMock).toHaveBeenCalledWith(
				"a",
		...
		it("Does not send invalid input when submitting", () => {
			...
			fireEvent.change(screen.getByRole("textbox", { name: "Url:" }), {
				target: { value: "https://www.yay.com/watch?v=ABCDEFGHIJK" },
			});
